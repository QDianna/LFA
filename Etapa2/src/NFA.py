...
class NFA[STATE]:
    S: set[str]                                 # alphabet
    K: set[STATE]                               # states
    q0: STATE                                   # inital state
    d: dict[tuple[STATE, str], set[STATE]]      # transitions
    ...
    def remap_states[OTHER_STATE](self, f: 'Callable[[STATE], OTHER_STATE]') -> 'NFA[OTHER_STATE]':
        ...
        new_NFA.K.update(f(state) for state in self.K)
        
        new_NFA.F.update(f(state) for state in self.F)
        
        new_NFA.d = {(f(state), symbol): {f(next_state) for next_state in next_states}
